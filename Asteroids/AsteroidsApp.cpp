...
{
	// this is a 'Lamda'  sort, the third parameter is a function
...
void AsteroidsApp::update(float deltaTime) {
	...
	if (input->isMouseButtonDown(0)) {
			...
			mShield.mPosX = (float)input->getMouseX();
			mShield.mPosY = (float)input->getMouseY();
			mShield.mLifetime = 6000;
			mShield.mLength = 120.0f;
			mShield.mRot = 0.0f;
			mShieldActive = true;
			mShield.mLifetime = 6000;
	...
	if (mShieldActive) {
		mShield.mLifetime -= deltaTime;
	}
	if (mShield.mLifetime == 0) {
		mShieldActive = false;
	}

	if (input->isMouseButtonDown(1)) {
		if (mPlayer.mCurrentXPos < input->getMouseX()) mPlayer.mCurrentXPos += mPlayer.mSpeed*deltaTime;
		if (mPlayer.mCurrentXPos > input->getMouseX()) mPlayer.mCurrentXPos -= mPlayer.mSpeed*deltaTime;
		if (mPlayer.mCurrentYPos < input->getMouseY()) mPlayer.mCurrentYPos += mPlayer.mSpeed*deltaTime;
		if (mPlayer.mCurrentYPos > input->getMouseY()) mPlayer.mCurrentYPos -= mPlayer.mSpeed*deltaTime;
		return;
	}
...
void AsteroidsApp::draw()
{
	...
	if (mShieldActive) {
		m_2dRenderer->drawBox(mShield.mPosX, mShield.mPosY, 120.0f, mShield.mLength, mShield.mRot, 0.0f);
	}
	...
	if (mGameIsOver) {
		...
		if (mScore) os << " [ " << mScore << " ] [ " << mShield.mLifetime << " ]";
